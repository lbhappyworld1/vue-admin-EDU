<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="课程封面" width="300px" align="center">
        <template slot-scope="{row}">
          <!-- <span>{{ row.course_cover}}</span> -->
          <div class="course_cover_img">
            <img :src="row.course_cover">
          </div>
        </template>
      </el-table-column>
      <el-table-column label="课程名称" width="150px" align="center">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.course_name }}</span>
          <!-- <el-tag>{{ row.type | typeFilter }}</el-tag> -->
        </template>
      </el-table-column>
      <el-table-column label="课程价格" width="150px" align="center">
        <template slot-scope="{row}">
          <span>￥{{ row.couser_price }}</span>
        </template>
      </el-table-column>
      <el-table-column label="购买人数" width="100px">
        <template slot-scope="{row}">
          <!-- <svg-icon v-for="n in + row.importance" :key="n" icon-class="star" class="meta-item__icon" /> -->
          <span>{{ row.couser_peoples }}人</span>
        </template>
      </el-table-column>
      <el-table-column label="推荐课程" align="center" width="200px">
        <template slot-scope="{row}">
          <span v-if="row.pageviews" class="link-type" @click="handleFetchPv(row.pageviews)">{{ row.pageviews }}</span>
          <span v-else>0</span>
        </template>
      </el-table-column>
      <el-table-column label="上课教师" class-name="status-col" width="200">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="472" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle } from '@/api/course'
