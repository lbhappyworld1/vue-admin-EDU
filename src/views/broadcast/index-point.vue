<template>
  <div class="content">
    ...
    >
      ...
      <el-table-column label="点播名称" width="550" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="点播时长" align="center">
        <template>
          <span>直播中</span>
        </template>
      </el-table-column>
      <el-table-column label="点播类型" align="center">
        <template slot-scope="{row}">
          <!-- <svg-icon v-for="n in + row.importance" :key="n" icon-class="star" class="meta-item__icon" /> -->
          <span>{{ row.couser_peoples }}财会专精-1.1入门指南</span>
        </template>
      </el-table-column>
      <el-table-column label="关联课时" align="center">
        <template slot-scope="{row}">
          <!-- <svg-icon v-for="n in + row.importance" :key="n" icon-class="star" class="meta-item__icon" /> -->
          <span>{{ row.couser_peoples }}财会专精-1.1入门指南</span>
        </template>
